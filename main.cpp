...
BOOL GameLoad(VOID)
{
	...
	//プレイ（サンプル）BGMを読み込み
	if (LoadAudio(&playBGM, ".\\Audio\\ネオンパープル.mp3", 128, DX_PLAYTYPE_LOOP) == FALSE) { return FALSE; }
...
VOID TitleProc(VOID)
{
	...
	}
	{
		...
		CollUpdateDivImage(&samplePlayerImg2);	//当たり判定の更新
	}
	*/
...
/*
int size = 10;
SetFontSize(size)とかで大きく小さく
SetFontSize(fontdef.size)でもとにも戻せる
*/
...
VOID TitleDraw(VOID)
{
	...
	DrawStringToHandle(500, 500, "しねきゃぷしょん", GetColor(0, 0, 0), capFont1.handle);
...
BOOL LoadImageMem(IMAGE* image, const char* path)
{
	...
	image->handle = LoadGraph(image->path);	//画像読み込み
